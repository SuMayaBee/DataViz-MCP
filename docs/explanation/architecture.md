# Understanding DataViz MCP

DataViz MCP enables AI assistants and developers to create and manage Python visualizations
through a dedicated local web server. This document explains the architecture, design decisions,
and key concepts.

## Architecture Overview

DataViz MCP uses a two-process architecture:
...
pls serve, Panel Server (subprocess, port 5077)
  │  SQLite  ~/.dataviz-mcp/snippets/snippets.db
  ▼
...
are determined by that environment rather than by DataViz MCP. You can add or upgrade
...
Snippets are stored in a SQLite database (default: `~/.dataviz-mcp/snippets/snippets.db`).
