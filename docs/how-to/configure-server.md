# Configure DataViz MCP

This guide shows you how to configure DataViz MCP, the Panel web server that executes
Python code snippets and renders interactive visualizations.

## Prerequisites

- DataViz MCP installed, see [Installation](../tutorials/installation.md)

---

## Default Configuration

DataViz MCP runs on `localhost` with sensible defaults. No configuration file is
required for local use.

| Setting | Default | Description |
|---|---|---|
| Port | per-environment (base `5077`) | Panel server port, derived from the active interpreter unless `DATAVIZ_MCP_PORT` is set. Each Python environment gets its own port so servers in different environments do not collide. |
| Host | `localhost` | Server host address |
| Database | `~/.dataviz-mcp/snippets/snippets.db` | SQLite database path |
...
```bash
export DATAVIZ_MCP_PORT=9999
export DATAVIZ_MCP_HOST=127.0.0.1
export DATAVIZ_MCP_DB_PATH=/data/my-snippets.db
export DATAVIZ_MCP_MAX_RESTARTS=5
```

**Windows (PowerShell):**

```powershell
$env:DATAVIZ_MCP_PORT="9999"
$env:DATAVIZ_MCP_HOST="127.0.0.1"
$env:DATAVIZ_MCP_DB_PATH="C:\data\my-snippets.db"
$env:DATAVIZ_MCP_MAX_RESTARTS="5"
```

**Windows (Command Prompt):**

```bat
set DATAVIZ_MCP_PORT=9999
set DATAVIZ_MCP_HOST=127.0.0.1
set DATAVIZ_MCP_DB_PATH=C:\data\my-snippets.db
set DATAVIZ_MCP_MAX_RESTARTS=5
```
...
To use DataViz MCP with AI assistants, start it in MCP mode:
...
DataViz MCP needs to know its public URL when running behind a proxy or tunnel so that
...
|---|---|
| `DATAVIZ_MCP_EXTERNAL_URL` | Any, explicit port-inclusive override |
...
```bash
export DATAVIZ_MCP_EXTERNAL_URL=<url-from-above>
```

**Windows (PowerShell):**

```powershell
$env:DATAVIZ_MCP_EXTERNAL_URL="<url-from-above>"
```
...
```bash
export DATAVIZ_MCP_EXTERNAL_URL="https://your-hub/user/you/proxy/5077"
```

**Windows (PowerShell):**

```powershell
$env:DATAVIZ_MCP_EXTERNAL_URL="https://your-hub/user/you/proxy/5077"
```
...
By default the SQLite database is stored at `~/.dataviz-mcp/snippets/snippets.db`.
To use a different location.

**macOS / Linux:**

```bash
export DATAVIZ_MCP_DB_PATH=/path/to/your/snippets.db
```

**Windows (PowerShell):**

```powershell
$env:DATAVIZ_MCP_DB_PATH="C:\path\to\your\snippets.db"
```
...
DataViz MCP automatically restarts the Panel subprocess if it becomes unhealthy, up to
...
- [API Reference](../reference/dataviz_mcp.md): full reference documentation
