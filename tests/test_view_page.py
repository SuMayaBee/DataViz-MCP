...
class TestExecuteCodeNoPreambleInjection:
    ...
    def test_jupyter_app_has_no_extension_call(self):
        ...
        from dataviz_mcp.pages.view_page import _execute_code
        ...
        with (
            patch("dataviz_mcp.pages.view_page.extract_last_expression", side_effect=fake_extract),
            patch("dataviz_mcp.pages.view_page.execute_in_module", return_value={}),
            patch("dataviz_mcp.pages.view_page.sys") as mock_sys,
            patch("dataviz_mcp.pages.view_page.pn"),
        ):
            mock_sys.modules = {}
    ...
    def test_panel_method_code_unchanged(self):
        ...
        with patch("dataviz_mcp.pages.view_page.execute_in_module", side_effect=fake_execute):
            _execute_code(snippet)
...
class TestCreateViewExtensionLoading:
    ...
    def test_plotly_extension_loaded_at_session_level(self, temp_db):
        ...
        from dataviz_mcp.pages.view_page import create_view

        snippet = temp_db.create_snippet(_make_snippet("x = 1  # plotly visualization"))
        pn_mock = MagicMock()

        with (
            patch("dataviz_mcp.pages.view_page.get_db", return_value=temp_db),
            patch("dataviz_mcp.pages.view_page.pn", pn_mock),
            patch("dataviz_mcp.pages.view_page._execute_code", return_value=MagicMock()),
        ):
